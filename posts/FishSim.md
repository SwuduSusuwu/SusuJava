...
* Improve the user interface (now is just [`fpsTextRefresh()`, which shows resource use](https://github.com/SwuduSusuwu/SusuJava/blob/d21ed39be6640e94f3091e934717e8c7cfbdc758/susuwu/SimUsages.java#L103-L143), plus [`renderFish()`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L323-L336)'s viewport which shows the models of `class Fish`), to allow to scroll the viewport around (requires [`resolution`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L159) to include 2 more `double`s, to store viewport offset, which `fish.isVisible` must use), plus allow to set numerous options (which for now have "functional constness" so those invariants allow `java` to use more efficient execution), plus allow the user to control one of the organisms (which for now are just `Fish`).
  * `FishSim`'s renderer (`renderFish`) is separate from [`FishSim`'s physics loop (`updateFish`)](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L307-L321) (`PhysicsMode physicsMode` allows to execute on separate **CPU**s through [`AnimationTimer`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L239-L244), [`Timeline`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L245-L251), or [`ExecutorService`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L269-L273)), but still shares the process with the physics loop.
    * Can [still execute `updateFish()` as part of `renderFish()` if `physicsMode = PhysicsMode.synchronous*`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L261-L265), but future versions are supposed to drop synchronous modes.
    ...
    * `updateFish()` is difficult to do at 24fps (the minimum which shows smooth motion to humans) with thousands of `Fish`. If `FishSim`'s [`monitorRefreshHertz`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L49) is > [`physicsRefreshHertz`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L50) (in particular, if `24 > physicsRefreshHertz`): sum `pos += dpos` on monitor refresh (or some sort of simple motion loop), so movement is smooth.
  * Introduce [servers](https://codingtechroom.com/question/create-basic-java-server) for shared experiences. But until `FishSim` has interfaces for user interactions, servers are limited to **RO** `./susuwu/PhysicsLoop.java` hosts with passive users. Due to variable latencies to / from servers, what those `./susuwu/PhysicsLoop.java` servers must do:
    * [`PhysicsMode physicsMode`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L50) = [`PhysicsMode.separateFps`](https://github.com/SwuduSusuwu/SusuJava/blob/702ecb81e2d65e845afcfb609eb9fb3ef105c6ec/susuwu/FishSim.java#L47).
...
* Introduce continuous goals for `class Fish` (with no predator / prey dynamics, though, goals are limited), whose progress is stored.
  ...
  * [./posts/MultiuserConcernsPlusGoals.md](./MultiuserConcernsPlusGoals.md) discusses numerous goals to pursue.
